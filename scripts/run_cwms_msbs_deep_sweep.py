...
Scope: lr, calibrated_lr (no_cleaning+cp only), hgb, xgboost, lightgbm, catboost
       x 5 datasets x 10 seeds x 3 hidden_minority protocols x up to 5 methods
...
FULL_METHODS = ["no_cleaning", "class_proportional", "msbs", "cwms", "cwms_msbs", "cwms_msbs_shuffled"]
...
XGBOOST_METHODS = ["no_cleaning", "class_proportional"]  # CWMS conflicts with scale_pos_weight
...
def _load_completed(path: Path) -> set:
    ...
    try:
        df = pd.read_csv(path)
        # Exclude error rows and NaN BA rows so they get retried
